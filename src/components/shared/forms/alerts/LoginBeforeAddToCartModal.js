...
const LoginBeforeAddToCartModal = () => {
  ...
  useEffect(() => {
    ...
    if (html) {
      if (state.showAddToCartButNotLoggedInModal && html) {
        html.style.overflowY = "hidden";

        const focusableElements =
          'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])';

        const modal = document.querySelector("#modal-not-logged-in"); // select the modal by it's [id]
  ...
  return (
    <>
      ...
      {state.showAddToCartButNotLoggedInModal && typeof document !== "undefined"
...
export default LoginBeforeAddToCartModal;
